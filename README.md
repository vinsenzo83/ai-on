...
| IP | 144.172.93.226 |
| SSH User | root |
| SSH Password | GjWZ9k2M3EWn6y |
| 앱 경로 | /opt/ai-orchestrator/app/ai-orchestrator |
| PM2 앱 이름 | ai-orchestrator |
| 어드민 URL | http://144.172.93.226/admin |
| 어드민 이메일 | admin@ai-orch.local |
| 어드민 비밀번호 | AiOrch2026!Secure |
| HTTPS | https://144.172.93.226 (자체서명 인증서) |

### 배포 명령어
```bash
# SSH 접속
sshpass -p "GjWZ9k2M3EWn6y" ssh root@144.172.93.226

# 코드 업데이트 + 재시작
cd /opt/ai-orchestrator/app && git pull origin genspark_ai_developer
cd /opt/ai-orchestrator/app/ai-orchestrator && npm ci --only=production --quiet
